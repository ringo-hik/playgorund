...
import { Project, ProjectSettings, BuildConfig, BuildType } from '../types/project';
import { MockSwdpClient } from './mockSwdpClient'; // Changed from MockSwdpClient

export class ProjectManager {
  private static instance: ProjectManager;
  private swdpClient: MockSwdpClient; // Changed type
  private currentProject: Project | null = null;
  private projectSettings: ProjectSettings | null = null;
  private readonly settingsFile = '.vscode/swdp-project.json';

  private constructor() {
    this.swdpClient = new MockSwdpClient(); // Instantiated SwdpClient
  }
  ...
  // 프로젝트 선택 UI 표시
  async selectProject(): Promise<Project | undefined> {
    try {
      const projects = await this.swdpClient.getProjects(); // Changed to getProjectList()
  ...
  // 프로젝트 설정 및 저장
  async setProject(project: Project): Promise<boolean> {
    try {
  ...
  // 프로젝트 설정 로드
  async loadProjectSettings(): Promise<boolean> {
    try {
      ...
      try {
        ...
        const settings = JSON.parse(content.toString()) as ProjectSettings;
  ...
  // 빌드 설정 수정
  async editBuildConfig(buildType: BuildType): Promise<void> {
  ...
  // 빌드 설정 편집 UI
  private async showBuildConfigEditor(config: BuildConfig, buildType: BuildType): Promise<BuildConfig | undefined> {
    const options = [
      {
  ...
  getBuildConfig(buildType: BuildType): BuildConfig | null {
    return this.projectSettings?.buildConfigs[buildType] || null;
  }

  getSwdpClient(): MockSwdpClient { // Changed return type
