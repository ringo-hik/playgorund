...
export class ProjectSettingsViewer {
  private static panels: Map<string, vscode.WebviewPanel> = new Map();

  static async showProjectSettingsInfo(project: Project): Promise<void> {
  ...
  private static getProjectSettingsHtml(project: Project): string {
  ...
  private static async updateProjectSetting(project: Project, key: string, value: any): Promise<void> {
    // 프로젝트 설정 업데이트 로직
    console.log(`Updating ${key} to ${value} for project ${project.id}`);
    // 실제 구현에서는 ProjectManager를 통해 설정 업데이트
  }

  private static async saveBuildConfig(project: Project, buildType: string, config: any): Promise<void> {
    // 빌드 설정 저장 로직
