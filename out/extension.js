...
const swdpTreeProvider_1 = require("./views/swdpTreeProvider");
const projectTreeProvider_1 = require("./views/projectTreeProvider");
const pumbaTreeProvider_1 = require("./views/pumbaTreeProvider");
const markdownViewer_1 = require("./utils/markdownViewer");
async function activate(context) {
    console.log('Build Agent extension is now active!');
    // 서비스 및 프로바이더 초기화
    const projectManager = projectManager_1.ProjectManager.getInstance();
    const swdpTreeProvider = new swdpTreeProvider_1.SwdpTreeProvider();
    const projectTreeProvider = new projectTreeProvider_1.ProjectTreeProvider();
    const pumbaTreeProvider = new pumbaTreeProvider_1.PumbaTreeProvider();
    // TreeView 등록
    ...
    const commands = [
        ...
        // 빌드 설정 명령어
        vscode.commands.registerCommand('buildAgent.editBuildConfig', async (buildType) => {
        ...
        // 기타 명령어
        vscode.commands.registerCommand('buildAgent.showBuildDetails', async (buildResult) => {
            await markdownViewer_1.MarkdownViewer.showBuildDetails(buildResult);
        }),
        vscode.commands.registerCommand('buildAgent.showHelp', async () => {
            await markdownViewer_1.MarkdownViewer.showHelp();
        }),
    ...
    // 구독 등록
    context.subscriptions.push(swdpTreeView, projectTreeView, pumbaTreeView, ...commands);
    // 상태바 항목 (선택적)
...
// 빌드 실행 함수
async function startBuild(buildType, projectManager, treeProvider) {
                    ...
                    if (status) {
                        ...
                        if (status.status === 'success' || status.status === 'failed') {
                            ...
                            if (status.status === 'success') {
                                vscode.window.showInformationMessage(`Build completed successfully in ${status.duration}`, 'View Details').then(selection => {
                                    if (selection === 'View Details') {
                                        markdownViewer_1.MarkdownViewer.showBuildDetails(buildResult);
                                    }
                                });
                            }
                            else {
                                vscode.window.showErrorMessage(`Build failed: ${status.errorMessage}`, 'View Details').then(selection => {
                                    if (selection === 'View Details') {
                                        markdownViewer_1.MarkdownViewer.showBuildDetails(buildResult);
                                    }
                                });
